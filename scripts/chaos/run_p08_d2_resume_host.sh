...
run_workers() {
  local executor_pid learner_pid
  ...
  wait "$learner_pid"
  wait "$executor_pid"
  touch "$sync_root/${member_id}_workers_done"
}
...
if [[ "$rank" -eq 0 ]]; then
  ...
  set -e
  [[ "$first_status" -ne 0 ]]
  "$PYTHON_BIN" - "$SHARED_ROOT/control/recoverable_error.json" "$ARTIFACT_ROOT/error_stop.json" <<'PY'
import json,sys
...
while [[ ! -f "$sync_root/error_stop_committed" ]]; do sleep 0.2; done
...
if [[ "$rank" -eq 1 ]]; then
  ...
  while [[ -f "$SHARED_ROOT/control/recoverable_error.json" ]]; do [[ "$SECONDS" -lt "$deadline" ]]; sleep 0.2; done
...
while [[ ! -f "$sync_root/member-000_workers_done" || ! -f "$sync_root/member-001_workers_done" ]]; do sleep 0.2; done
